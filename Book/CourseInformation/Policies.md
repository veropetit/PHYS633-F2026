...
Students are **not allowed to use advanced automated tools** (such as LLMs or Dall-E) on assignments in this course (unless explicitely specified). Each student is expected to complete each assignment without substantive assistance from others, including automated tools.

Some reasonable exceptions to the "no-AI": 

This is not a coding course and I already assume some familiarity with python. We are using coding as a *tool to learn* about stellar astropysics concepts.

The idea is to use code that is as simple as possible -- AI code output, when given a task, will often *too professional* (!!) for our needs (for example, there is no need to create a whole object-oriented class for our simple calculations). 

This said, if your (self-written!) code crashes and you cannot find the source of the error after 10 minutes: copy-paste the error message in the Gemini available with your UD account and ask why the code crashes. (I prefer that you use the Gemini available with your UD account, as it is not supposed to train on the prompts and results). 

Also, if you google things like "how to I change the color of the curve in matplotlib", it will give an AI answer -- this is perfectly fine. I google (small informational) things all the time.
...
* be mindful of cultural norms. Make sure you know how your classmates want to be addressed. Do not interrupt your classmates while they are speaking, do not speak over classmates and be aware and respectful of the natural volume of their voice. Be respectful of individual communication skills and fluency in the common languages we speak.
